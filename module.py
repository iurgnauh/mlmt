...
class Highway(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in range(self.num_layers):
            #print ("shape char embedding: ", x.shape)
...
class Embedding(nn.Embedding):
    def __init__(self, conf):
        ...
        #only has it when it translation from source language
        #merge the two embedding
        self.source_embedding_path =  getattr(conf, 'source_embedding_path', None)
    ...
    def load(self, path, vocab, stats=False):
        ...
        count = 0
        #add set to keep track of visited words
        visited = set()
        with open(path, 'r', encoding='utf-8') as r:
            ...
            try:
                ...
                print ("The OOV rate is: ",1 - float(count)/len(vocab))
            except UnicodeDecodeError as e:
                print(e)

        #only add it for translation task
        #add visited here? priority the target word embedding
        #what if we have token overlap?
        if self.source_embedding_path:
            logger.info('Loading embedding from source language{}'.format(self.source_embedding_path))
            with open(self.source_embedding_path, 'r', encoding='utf-8') as r:
                if stats:
                    r.readline()
                try:
                    for line in r:
                        line = line.strip().split(' ')
                        token = line[0]
                        if token in vocab and token not in visited:
                            vector = self.weight.data.new(
                                [float(v) for v in line[1:]])
                            self.weight.data[vocab[token]] = vector
                            #count the number of match
